...
test_that("eegSlowing classifies normal EEG correctly", {
  # Seed for determinism: the Welch band-power estimator resolves make_eeg's
  # random-walk delta more sharply than the old single-window FFT, so the
  # DTAR classification of this synthetic signal is borderline without a seed.
  set.seed(1)
  pe <- make_eeg(n_time = 5000, n_channels = 19, sr = 500)
