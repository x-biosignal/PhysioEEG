...
test_that("eegWPLI returns correct structure with wPLI in [0, 1]", {
  ...
  # Debiased wPLI (Vinck 2011, Eq. 6) is the unbiased estimator: it is a finite
  # value <= 1 and may be negative for weak/absent connectivity (not clamped).
  expect_true(all(is.finite(wpli_df$wpli_debiased)))
  expect_true(all(wpli_df$wpli_debiased <= 1 + 1e-9, na.rm = TRUE))
})
