...
Applies a windowed-sinc FIR bandpass filter using a Hamming window. The
...
``` r
.fir_bandpass(signal, sr, low, high, order = NULL)
```
...
- order:
  ...
  the lowest frequency, clamped to signal length and forced odd.
