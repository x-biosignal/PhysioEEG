...
#'   \code{"robust"} (PREP robust average reference: iteratively detect bad
#'   channels and exclude them from the average, Bigdely-Shamlo et al., 2015),
#'   \code{"median"} (channel-wise median reference, robust to outlier channels),
...
#' @param robust_noise_sd Noise threshold (SDs above the median channel
#'   variance) for bad-channel detection in \code{ref_type = "robust"}
#'   (default: 4).
#' @param robust_max_iter Maximum PREP iterations for \code{ref_type = "robust"}
#'   (default: 5).
#'
#' @return A PhysioExperiment object with re-referenced data in the specified
#'   output assay. Stores reference info (including the channels excluded by the
#'   robust reference) in \code{metadata(x)$reference} and logs the step in the
#'   object's provenance.
#' @references
#' Bigdely-Shamlo, N., et al. (2015). "The PREP pipeline: standardized
#' preprocessing for large-scale EEG analysis." \emph{Frontiers in
#' Neuroinformatics}, 9, 16. \doi{10.3389/fninf.2015.00016}
#' @export
#' @examples
#' \dontrun{
#' pe <- make_eeg(n_time = 5000, n_channels = 19, sr = 500)
...
#' }
eegRereference <- function(x, ref_type = c("average", "robust", "median",
                                            "mastoids", "cz", "rest", "channel"),
  ...
  if (ref_type == "average") {
    ...
    # PREP robust average reference (Bigdely-Shamlo et al., 2015): iteratively
    # detect bad (noisy) channels and exclude them from the average, so a few
    # high-amplitude channels cannot contaminate the reference.
    bad_set <- character(0)
    for (iter in seq_len(robust_max_iter)) {
      tmp <- x
      SummarizedExperiment::assay(tmp, ".robust_ref_tmp") <- result
      bd <- eegBadChannels(tmp, method = "noise",
                           assay_name = ".robust_ref_tmp",
                           noise_threshold = robust_noise_sd)
      new_bad <- bd$channel[bd$is_bad]
      if (setequal(new_bad, bad_set)) break
      bad_set <- new_bad
      good_idx <- which(!(labels %in% bad_set))
      if (length(good_idx) == 0) {
        stop("Robust reference excluded all channels; loosen robust_noise_sd.",
             call. = FALSE)
      }
      avg <- rowMeans(data[, good_idx, drop = FALSE])
      for (ch in seq_len(n_ch)) result[, ch] <- data[, ch] - avg
    }
    excluded_channels <- bad_set

  } else if (ref_type == "median") {
    # Channel-wise median reference: subtract the per-timepoint median across
    # channels (robust to outlier channels).
    ref_signal <- apply(data, 1, stats::median)
    for (ch in seq_len(n_ch)) {
  ...
  x <- PhysioCore::withProvenance(x, x, step = "eegRereference",
...
# ---- autoreject internals --------------------------------------------------

#' Peak-to-peak amplitude per channel and epoch (internal)
#'
#' @param data 3D array (time x channels x epochs).
#' @return A channels x epochs matrix of peak-to-peak amplitudes.
#' @keywords internal
.autoreject_ptp <- function(data) {
  apply(data, c(2, 3), function(v) max(v) - min(v))
}

#' Interpolate given channels of one epoch slice via spherical spline (internal)
#'
#' Repairs the \code{bad_idx} channels of a 2D epoch slice
#' (time x channels) using the same spherical-spline weights as
#' [eegInterpolate()], caching each good/bad configuration's weight matrix.
#'
#' @param slice A time x channels matrix.
#' @param bad_idx Integer indices of channels to interpolate.
#' @param pos Electrode positions matrix (channels x 3).
#' @param cache Optional environment used to memoise weight matrices.
#' @return The slice with \code{bad_idx} channels replaced by interpolation.
#' @keywords internal
.autoreject_interp <- function(slice, bad_idx, pos, cache = NULL) {
  if (length(bad_idx) == 0) return(slice)
  good_idx <- setdiff(seq_len(ncol(slice)), bad_idx)
  if (length(good_idx) < 3) return(slice)   # too few good channels to repair
  key <- paste(bad_idx, collapse = ",")
  w <- if (!is.null(cache)) cache[[key]] else NULL
  if (is.null(w)) {
    w <- .spherical_spline_weights(pos[good_idx, , drop = FALSE],
                                   pos[bad_idx, , drop = FALSE])
    if (!is.null(cache)) cache[[key]] <- w
  }
  slice[, bad_idx] <- slice[, good_idx, drop = FALSE] %*% t(w)
  slice
}
...
#'
#' @param data 3D array (time x channels x epochs).
#' @param ptp Channels x epochs peak-to-peak matrix.
#' @param grid Candidate threshold vector.
#' @param folds Integer fold assignment per epoch.
#' @return Numeric vector of per-channel thresholds.
#' @keywords internal
.autoreject_cv_thresholds <- function(data, ptp, grid, folds) {
  n_time <- dim(data)[1]; n_ch <- dim(data)[2]
  k <- max(folds)
  test_by_fold <- lapply(seq_len(k), function(f) which(folds == f))
  train_by_fold <- lapply(seq_len(k), function(f) which(folds != f))
  tau <- numeric(n_ch)
  for (ch in seq_len(n_ch)) {
    med_test <- lapply(seq_len(k), function(f)
      apply(matrix(data[, ch, test_by_fold[[f]]], nrow = n_time), 1, stats::median))
    err <- vapply(grid, function(th) {
      fe <- vapply(seq_len(k), function(f) {
        train <- train_by_fold[[f]]
        good <- train[ptp[ch, train] <= th]
        if (length(good) < 1) return(Inf)
        mu <- rowMeans(matrix(data[, ch, good], nrow = n_time))
        sqrt(mean((mu - med_test[[f]])^2))
      }, numeric(1))
      mean(fe)
    }, numeric(1))
    tau[ch] <- grid[which.min(err)]
  }
  tau
}
...
#' @param data 3D array (time x channels x epochs).
#' @param ptp Channels x epochs peak-to-peak matrix.
#' @param tau Per-channel thresholds.
#' @param pos Electrode positions matrix.
#' @param kappa_grid Candidate consensus fractions.
#' @param rho_grid Candidate interpolation counts.
#' @param folds Integer fold assignment per epoch.
#' @return List with \code{consensus}, \code{n_interpolate}, and \code{err}.
#' @keywords internal
.autoreject_select_consensus <- function(data, ptp, tau, pos, kappa_grid,
                                         rho_grid, folds) {
  n_time <- dim(data)[1]; n_ch <- dim(data)[2]; n_ep <- dim(data)[3]
  k <- max(folds)
  bad <- ptp > matrix(tau, n_ch, n_ep)
  test_by_fold <- lapply(seq_len(k), function(f) which(folds == f))
  train_by_fold <- lapply(seq_len(k), function(f) which(folds != f))
  med_by_fold <- lapply(seq_len(k), function(f)
    apply(data[, , test_by_fold[[f]], drop = FALSE], c(1, 2), stats::median))
  cache <- new.env(parent = emptyenv())
  best <- list(err = Inf, consensus = kappa_grid[1], n_interpolate = rho_grid[1])
  for (kappa in kappa_grid) for (rho in rho_grid) {
    fe <- vapply(seq_len(k), function(f) {
      accum <- matrix(0, n_time, n_ch); nkeep <- 0L
      for (e in train_by_fold[[f]]) {
        b <- which(bad[, e])
        if (length(b) / n_ch >= kappa) next
        slice <- data[, , e]
        if (length(b) > 0 && rho > 0) {
          worst <- b[order(ptp[b, e], decreasing = TRUE)][seq_len(min(rho, length(b)))]
          slice <- .autoreject_interp(slice, worst, pos, cache)
        }
        accum <- accum + slice; nkeep <- nkeep + 1L
      }
      if (nkeep == 0L) return(Inf)
      sqrt(mean((accum / nkeep - med_by_fold[[f]])^2))
    }, numeric(1))
    e <- mean(fe)
    if (is.finite(e) && e < best$err)
      best <- list(err = e, consensus = kappa, n_interpolate = rho)
  }
  best
}
...
#' dropped, according to two hyperparameters: \code{consensus} (the fraction of
#' bad channels above which an epoch is dropped) and \code{n_interpolate} (the
...
#' [eegMontage()] first).
#'
#' The cross-validation objective, at every stage, is the root-mean-square error
#' between the mean of the good/cleaned training epochs and the median of the
#' held-out test epochs - the criterion introduced by autoreject.
#'
#' @param x A PhysioExperiment with 3D epoched data (time x channels x epochs)
#'   in \code{metadata(x)[[assay_name]]} (from [eegEpoch()]) or in a 3D assay of
#'   that name (from [make_eeg_erp()]).
#' @param consensus Fraction of bad channels (from 0 to 1) above which an epoch
#'   is dropped. \code{NULL} selects it by cross-validation.
#' @param n_interpolate Number of worst bad channels to interpolate per repaired
#'   epoch. \code{NULL} selects it by cross-validation.
#' @param cv_folds Number of cross-validation folds (default: 5).
#' @param thresh_range Optional candidate peak-to-peak threshold grid: either a
#'   length-2 \code{c(min, max)} span (expanded to \code{n_thresh} values) or an
#'   explicit vector of candidate thresholds. \code{NULL} derives the span from
#'   the data.
#' @param n_thresh Number of grid thresholds when \code{thresh_range} is
#'   \code{NULL} or a span (default: 20).
#' @param assay_name Name of the 3D epoched data in metadata or assays
#'   (default: \code{"epoched"}).
#' @param output_assay Name for the cleaned 3D data stored in metadata
#'   (default: \code{"autoreject_clean"}).
#' @return The PhysioExperiment with cleaned 3D epoched data in
#'   \code{metadata(x)[[output_assay]]} (dropped epochs removed, repaired epochs
#'   interpolated) and a rejection log in \code{metadata(x)$autoreject}: the
#'   per-channel \code{thresholds}, chosen \code{consensus} and
#'   \code{n_interpolate}, the epoch-by-channel \code{bad_matrix}, the
#'   \code{interpolated} channel-epochs, and the \code{dropped_epochs}. The step
...
#' 159, 417-429.
#' @seealso [eegArtifactReject()], [eegInterpolate()], [eegEpoch()],
#'   [eegMontage()]
#' @export
#' @examples
#' \dontrun{
...
#' }
eegAutoReject <- function(x, consensus = NULL, n_interpolate = NULL,
                          cv_folds = 5L, thresh_range = NULL, n_thresh = 20L,
                          assay_name = "epoched", output_assay = "autoreject_clean") {
  stopifnot(inherits(x, "PhysioExperiment"))

  # --- resolve 3D epoched data (metadata first, then a 3D assay) ---
  data <- S4Vectors::metadata(x)[[assay_name]]
  if (is.null(data) && assay_name %in% SummarizedExperiment::assayNames(x)) {
    cand <- SummarizedExperiment::assay(x, assay_name)
    if (length(dim(cand)) == 3) data <- cand
  }
  if (is.null(data) || length(dim(data)) != 3) {
    stop("eegAutoReject requires 3D epoched data (time x channels x epochs) in ",
         "metadata or a 3D assay. Run eegEpoch() first.", call. = FALSE)
  }
  n_time <- dim(data)[1]; n_ch <- dim(data)[2]; n_ep <- dim(data)[3]

  cv_folds <- as.integer(cv_folds)
  if (cv_folds < 2L || cv_folds > n_ep) {
    stop("cv_folds must be between 2 and the number of epochs (", n_ep, ").",
         call. = FALSE)
  }
  if (!is.null(consensus) && (consensus < 0 || consensus > 1)) {
    stop("consensus must be a fraction from 0 to 1.", call. = FALSE)
  }

  # --- electrode positions (needed for interpolation) ---
  cd <- SummarizedExperiment::colData(x)
  labels <- as.character(cd$label)
  pos_cols <- c("pos_x", "pos_y", "pos_z")
  if (!all(pos_cols %in% colnames(cd))) {
    stop("Electrode positions (pos_x, pos_y, pos_z) are required in colData. ",
         "Apply eegMontage() first.", call. = FALSE)
  }
  pos <- as.matrix(cd[, pos_cols])

  # --- peak-to-peak matrix, threshold grid, deterministic interleaved folds ---
  ptp <- .autoreject_ptp(data)
  if (is.null(thresh_range)) {
    grid <- seq(min(ptp), max(ptp), length.out = n_thresh)
  } else if (length(thresh_range) == 2) {
    grid <- seq(min(thresh_range), max(thresh_range), length.out = n_thresh)
  } else {
    grid <- sort(unique(thresh_range))
  }
  folds <- ((seq_len(n_ep) - 1L) %% cv_folds) + 1L

  # --- Stage 1: per-channel CV thresholds ---
  tau <- .autoreject_cv_thresholds(data, ptp, grid, folds)

  # --- Stage 2: consensus + n_interpolate (CV unless both supplied) ---
  kappa_grid <- seq(0.1, 1, by = 0.1)
  rho_grid <- 0:min(4L, n_ch - 4L)
  if (is.null(consensus) || is.null(n_interpolate)) {
    sel <- .autoreject_select_consensus(data, ptp, tau, pos, kappa_grid,
                                        rho_grid, folds)
    kappa <- if (is.null(consensus)) sel$consensus else consensus
    rho <- if (is.null(n_interpolate)) sel$n_interpolate else as.integer(n_interpolate)
  } else {
    kappa <- consensus; rho <- as.integer(n_interpolate)
  }

  # --- Stage 3: apply thresholds + consensus/interpolation ---
  bad <- ptp > matrix(tau, n_ch, n_ep)
  cache <- new.env(parent = emptyenv())
  keep <- logical(n_ep)
  interp_list <- vector("list", n_ep)
  out <- array(NA_real_, dim = c(n_time, n_ch, n_ep))
  for (e in seq_len(n_ep)) {
    b <- which(bad[, e])
    if (length(b) / n_ch >= kappa) { keep[e] <- FALSE; next }
    keep[e] <- TRUE
    slice <- data[, , e]
    if (length(b) > 0 && rho > 0) {
      worst <- b[order(ptp[b, e], decreasing = TRUE)][seq_len(min(rho, length(b)))]
      slice <- .autoreject_interp(slice, worst, pos, cache)
      interp_list[[e]] <- labels[worst]
    }
    out[, , e] <- slice
  }
  cleaned <- out[, , keep, drop = FALSE]
  dropped <- which(!keep)

  # --- rejection log + provenance ---
  bad_matrix <- t(bad)
  rownames(bad_matrix) <- paste0("epoch", seq_len(n_ep))
  colnames(bad_matrix) <- labels
  interp_df <- do.call(rbind, lapply(which(keep), function(e) {
    ch <- interp_list[[e]]
    if (length(ch) == 0) NULL else
      data.frame(epoch = e, channel = ch, stringsAsFactors = FALSE)
  }))
  if (is.null(interp_df)) {
    interp_df <- data.frame(epoch = integer(0), channel = character(0),
                            stringsAsFactors = FALSE)
  }
  names(tau) <- labels
  md <- S4Vectors::metadata(x)
  ...
  if (length(dropped) > 0 || nrow(interp_df) > 0) {
    message(sprintf(
      "autoreject: dropped %d of %d epochs, interpolated %d channel-epochs (consensus=%.2f, n_interpolate=%d).",
      length(dropped), n_ep, nrow(interp_df), kappa, rho))
  }

  x <- PhysioCore::withProvenance(x, x, step = "eegAutoReject",
                                  params = list(consensus = kappa,
                                                n_interpolate = rho,
                                                cv_folds = cv_folds,
