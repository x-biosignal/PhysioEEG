...
make_eeg_bci <- function(n_trials = 30, n_channels = 8, sr = 256,
  ...
  ch_pool <- c("C3", "Cz", "C4", "P3", "Pz", "P4", "O1", "O2")
  # Pad with generic labels when more channels than the motor/occipital pool are
  # requested, so ch_labels never contains NA (an NA label would make the
  # per-channel `if (ch_labels[ch] == ...)` tests below error).
  ch_labels <- if (n_channels <= length(ch_pool)) {
    ch_pool[seq_len(n_channels)]
  } else {
    c(ch_pool, paste0("EEG", seq_len(n_channels - length(ch_pool))))
  }
