...
#' Prepare continuous EEG data for butterfly and GFP panels
#'
#' @param x A PhysioExperiment object.
#' @param channels Character channel labels or \code{NULL}.
#' @param time_range Display range in seconds or \code{NULL}.
#' @param show_events Whether to include event markers.
#' @param assay_name Assay name or \code{NULL}.
#' @param gfp_definition GFP denominator definition.
#' @param heights Relative panel heights.
#' @return Validated panel data and settings.
#' @keywords internal
#' @noRd
.prepare_butterfly_gfp <- function(
    x,
    channels,
    time_range,
    show_events,
  ...
  gfp_definition <- match.arg(gfp_definition, c("population", "sample"))

  if (!is.logical(show_events) || length(show_events) != 1L ||
      is.na(show_events)) {
    stop("show_events must be one non-missing logical value.", call. = FALSE)
  }
  if (!is.numeric(heights) || length(heights) != 2L ||
      any(!is.finite(heights)) || any(heights <= 0)) {
    stop("heights must be two finite positive numbers.", call. = FALSE)
  }
  if (!is.null(time_range) &&
      (!is.numeric(time_range) || length(time_range) != 2L ||
       any(!is.finite(time_range)) || time_range[1L] >= time_range[2L])) {
    stop("time_range must be two finite, strictly increasing numbers.",
         call. = FALSE)
  }
  ...
  if (!is.matrix(data) || !is.numeric(data) ||
    ...
    stop("The selected assay must be a non-empty numeric 2D matrix ",
         "(time x channels).", call. = FALSE)
  }

  sr <- samplingRate(x)
  if (!is.numeric(sr) || length(sr) != 1L || !is.finite(sr) || sr <= 0) {
    stop("samplingRate(x) must be one finite positive number.", call. = FALSE)
  }
  ...
  if (length(ch_labels) != ncol(data) ||
      anyNA(ch_labels) || any(!nzchar(ch_labels))) {
    stop("Channel labels must be non-missing, non-empty, and match the assay.",
         call. = FALSE)
  }

  if (is.null(channels)) {
    ch_idx <- seq_len(ncol(data))
  } else {
    if (!is.character(channels) || length(channels) < 1L ||
        anyNA(channels) || any(!nzchar(channels)) || anyDuplicated(channels)) {
      stop("channels must be a non-empty vector of unique exact labels.",
           call. = FALSE)
    }
    duplicated_labels <- unique(ch_labels[duplicated(ch_labels) |
                                            duplicated(ch_labels,
                                                       fromLast = TRUE)])
    if (length(intersect(channels, duplicated_labels)) > 0L) {
      stop("Requested channel labels must occur exactly once in colData(x).",
           call. = FALSE)
    }
    ch_idx <- match(channels, ch_labels)
    if (anyNA(ch_idx)) {
      missing_channels <- channels[is.na(ch_idx)]
      stop("Unknown channel label(s): ",
           paste(missing_channels, collapse = ", "), ".", call. = FALSE)
    }
  }

  selected <- data[, ch_idx, drop = FALSE]
  if (any(!is.finite(selected))) {
    stop("Selected EEG data must contain only finite values.", call. = FALSE)
  }
  if (gfp_definition == "sample" && ncol(selected) < 2L) {
    stop("Sample GFP requires at least two selected channels.", call. = FALSE)
  }
  if (gfp_definition == "population" && ncol(selected) == 1L) {
    warning("Population GFP for one channel is zero and is not spatially ",
            "informative.", call. = FALSE)
  }

  time <- (seq_len(nrow(selected)) - 1L) / sr
  time_idx <- if (is.null(time_range)) {
    seq_along(time)
  } else {
    which(time >= time_range[1L] & time <= time_range[2L])
  }
  if (length(time_idx) < 1L) {
    stop("time_range does not contain any recorded samples.", call. = FALSE)
  }

  gfp <- .eeg_global_field_power(selected, definition = gfp_definition)
  display_time <- time[time_idx]
  display_labels <- ch_labels[ch_idx]
  butterfly <- data.frame(
    time = rep(display_time, times = length(ch_idx)),
    amplitude = as.vector(selected[time_idx, , drop = FALSE]),
    channel = rep(display_labels, each = length(time_idx)),
    channel_id = rep(seq_along(ch_idx), each = length(time_idx)),
    stringsAsFactors = FALSE
  )
  butterfly$channel <- factor(butterfly$channel,
                              levels = unique(display_labels))

  gfp_data <- data.frame(
    time = display_time,
    gfp = gfp[time_idx],
    definition = gfp_definition,
    stringsAsFactors = FALSE
  )

  event_data <- data.frame(time = numeric())
  if (show_events) {
    events <- S4Vectors::metadata(x)$events
    if (!is.null(events) &&
        (is.data.frame(events) || inherits(events, "DataFrame"))) {
      event_col <- if ("time" %in% names(events)) {
        "time"
      } else if ("time_sec" %in% names(events)) {
        "time_sec"
      } else if ("onset" %in% names(events)) {
        "onset"
      } else {
        NULL
      }
      if (!is.null(event_col)) {
        ...
        if (!is.numeric(event_times) || any(!is.finite(event_times))) {
          stop("Event times must contain only finite numeric values.",
               call. = FALSE)
        }
        display_limits <- range(display_time)
        event_times <- event_times[
          event_times >= display_limits[1L] &
            event_times <= display_limits[2L]
        ]
        event_data <- data.frame(time = event_times)
      }
    }
  }
  ...
  list(
    ...
    gfp = gfp_data,
    events = event_data,
    time_limits = range(display_time),
    definition = gfp_definition,
...
#'
#' @param x A PhysioExperiment object containing a numeric 2D continuous EEG
#'   assay (time x channels).
#' @param channels Unique character vector of exact channel labels. If
#'   \code{NULL}, all channels are shown.
#' @param time_range Optional finite increasing length-two vector specifying
#'   the inclusive display range in seconds. GFP is calculated before display
#'   windowing.
#' @param show_events Logical; draw finite event times from
#'   \code{metadata(x)$events} on both panels.
#' @param assay_name Input assay name. If \code{NULL}, uses the default assay.
#' @param gfp_definition Denominator used for spatial dispersion:
#'   \code{"population"} divides by C and matches \code{eegMicrostates()};
#'   \code{"sample"} divides by C - 1.
#' @param heights Two finite positive values giving the butterfly and GFP
#'   panel height ratio.
...
eegPlotButterflyGFP <- function(
  ...
  if (!requireNamespace("patchwork", quietly = TRUE)) {
    stop("Package 'patchwork' is required for eegPlotButterflyGFP(). ",
         "Install with install.packages('patchwork').", call. = FALSE)
  }

  prepared <- .prepare_butterfly_gfp(
    x = x,
    channels = channels,
    time_range = time_range,
    show_events = show_events,
    assay_name = assay_name,
    gfp_definition = gfp_definition,
    heights = heights
  )
  ...
  upper <- ggplot2::ggplot(
    ...
    PhysioCore::scale_color_physio(drop = FALSE) +
    ggplot2::scale_x_continuous(limits = prepared$time_limits) +
    ggplot2::labs(
      x = NULL,
      y = "Amplitude",
      color = "Channel",
  ...
  definition_label <- if (prepared$definition == "population") {
    "Population denominator C"
  } else {
    "Sample denominator C - 1"
  }
  lower <- ggplot2::ggplot(
    prepared$gfp,
    ggplot2::aes(x = .data$time, y = .data$gfp)
  ) +
    ggplot2::geom_line(linewidth = 0.65, color = "#3A5568") +
    ggplot2::scale_x_continuous(limits = prepared$time_limits) +
    ggplot2::labs(
      x = "Time (s)",
      y = "GFP",
  ...
  if (nrow(prepared$events) > 0L) {
    event_layer <- ggplot2::geom_vline(
    ...
    upper <- upper + event_layer
    lower <- lower + event_layer
  }

  patchwork::wrap_plots(upper, lower, ncol = 1L) +
...
eegPlotERP <- function(x, channels = NULL, conditions = NULL,
  ...
  p <- p +
    ...
    PhysioCore::scale_color_physio(drop = FALSE) +
    PhysioCore::scale_fill_physio(drop = FALSE) +
    ggplot2::labs(x = "Time (s)", y = "Amplitude",
...
#'
#' Displays a time-frequency representation of EEG data as a heatmap. It uses
#' an unambiguous stored STFT, Morlet, or ERSP product when available. For a
#' continuous 2D assay, it computes the established sliding-window FFT.
#'
#' @param x A PhysioExperiment object with EEG data.
...
#' @param assay_name Input assay or time-frequency metadata product name. If
#'   \code{NULL}, uses one unambiguous STFT, Morlet, or ERSP product when
#'   present, otherwise computes the legacy sliding FFT from the default assay.
#' @param mask Optional display mask. Supply a logical time-by-frequency matrix,
#'   a p-value matrix with an explicit \code{alpha} attribute, or a private
...
#' @param mask_alpha Opacity for bins outside \code{mask}, as one finite value
#'   in \code{[0, 1]} (default: 0.4).
#' @param contour Logical; draw non-interpolated boundaries between included
#'   and excluded mask bins when both are present (default: TRUE).
#' @return A ggplot2 object.
#' @details A mask controls opacity and optional boundaries only. It never
#'   replaces, zeros, or otherwise changes the plotted power values. Matrix
#'   masks describe the complete time-by-frequency grid before
#'   \code{freq_range} or \code{time_range} is applied.
#'
#'   Statistical masks must be computed from repeated, independent and
#'   exchangeable observations, not inferred from the single spectrogram being
#'   plotted. The package-private cluster helper subtracts the declared
...
eegPlotSpectrogram <- function(x, channel = 1, freq_range = NULL,
                               ...
                               mask = NULL, mask_alpha = 0.4,
                               contour = TRUE) {
  .check_ggplot2()
  if (!is.numeric(mask_alpha) || length(mask_alpha) != 1L ||
      is.na(mask_alpha) || !is.finite(mask_alpha) ||
      mask_alpha < 0 || mask_alpha > 1) {
    stop("'mask_alpha' must be one finite number in [0, 1].", call. = FALSE)
  }
  if (!is.logical(contour) || length(contour) != 1L || is.na(contour)) {
    stop("'contour' must be TRUE or FALSE.", call. = FALSE)
  }
  validate_range <- function(value, label) {
    if (is.null(value)) {
      return(NULL)
    }
    if (!is.numeric(value) || length(value) != 2L ||
      ...
      stop(
        sprintf("'%s' must be NULL or two increasing finite values.", label),
        call. = FALSE
      )
    }
    value
  }
  freq_range <- validate_range(freq_range, "freq_range")
  time_range <- validate_range(time_range, "time_range")
  extracted <- .eeg_extract_time_frequency(
    x,
    channel = channel,
    assay_name = assay_name,
    log_power = log_power
  )
  tf_matrix <- extracted$power
  time_vec <- extracted$time
  freqs <- extracted$frequency
  display_mask <- .eeg_tf_prepare_mask(mask, time_vec, freqs)
  ...
  if (!is.null(display_mask)) {
    plot_df$significant <- as.vector(display_mask)
    plot_df$display_alpha <- ifelse(
      plot_df$significant,
      1,
      mask_alpha
    )
  ...
  p <- p +
    ggplot2::scale_fill_viridis_c(option = sub("viridis", "D", palette),
                                  name = extracted$settings$power_label) +
    ggplot2::labs(x = "Time (s)", y = "Frequency (Hz)",
                  title = "Spectrogram") +
    PhysioCore::theme_physio()

  if (!is.null(display_mask) && contour) {
    time_indices <- if (is.null(time_range)) {
      seq_along(time_vec)
    } else {
      which(time_vec >= time_range[1L] & time_vec <= time_range[2L])
    }
    frequency_indices <- if (is.null(freq_range)) {
      seq_along(freqs)
    } else {
      which(freqs >= freq_range[1L] & freqs <= freq_range[2L])
    }
    if (length(time_indices) >= 2L && length(frequency_indices) >= 2L) {
      boundaries <- .eeg_tf_mask_boundaries(
        display_mask[
          time_indices,
          frequency_indices,
          drop = FALSE
        ],
        time_vec[time_indices],
        freqs[frequency_indices]
      )
...
#' projections from source amplitudes with real three-dimensional coordinates.
#' The bundled outline is schematic and is not an MRI, patient anatomy, or a
#' validation of the inverse solution.
#'
#' @param x A PhysioExperiment object.
#' @param source_data Optional explicit source data. Supply a data frame with
#'   finite \code{x}, \code{y}, \code{z}, and \code{amplitude} columns, or a
#'   structured list with \code{positions} and declared amplitude/matrix
#'   reduction fields. If \code{NULL}, uses coordinate-bearing metadata from a
#'   new \code{eegSourceEstimate()} or \code{eegBeamformer()} result.
#' @param views Non-empty unique character vector drawn exactly from
#'   \code{"sagittal"}, \code{"axial"}, and \code{"coronal"}. Request order is
#'   preserved.
#' @param threshold_pct Finite percentile in \code{[0, 100]}. The threshold is
#'   the type-8 quantile of absolute amplitude over the complete source set;
#'   ties are retained.
#' @return A patchwork object for normal projections. When fewer than three
#'   rows survive thresholding, returns the first requested view as an
#'   \code{eegPlotSource()} ggplot fallback without applying a second
#'   threshold. Immutable \code{"glassbrain_data"} attributes retain the
#'   resolved sources, panel data, display transform, and threshold settings.
#' @details Projection axes are sagittal \code{y-z} with depth \code{x}, axial
#'   \code{x-y} with depth \code{z}, and coronal \code{x-z} with depth
#'   \code{y}. Free-orientation source estimates are reduced by root-sum-square
#'   over orientations at each time and RMS over time. This non-negative
#'   summary is not a signed instantaneous current. A fixed 80 by 80 display
#'   grid retains the greatest absolute amplitude in each projected bin, with
#'   exact ties resolved by the smallest source ID. The affine display
#'   transform comes from the complete unthresholded coordinate cloud and does
#'   not modify amplitudes. The bundled outline is schematic: the plot is not
#'   MRI registration, patient anatomy, or validation of localization accuracy.
#'   Bare amplitude vectors and two-dimensional coordinates are rejected.
#' @seealso [eegForwardModel()], [eegSourceEstimate()], [eegBeamformer()],
#'   [eegPlotSource()]
#' @export
#' @examples
#' \dontrun{
