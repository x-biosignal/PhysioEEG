...
#' ICLabel-style seven-class classifier ([eegICLabel()]). The \code{"iclabel"}
#' method delegates to [eegICLabel()] and collapses its result to the
#' two-class contract (any non-brain argmax is an \code{"artifact"}), with the
#' score being one minus the brain probability.
#'
#' @param x A PhysioExperiment object with ICA results (from \code{eegICA}).
#' @param method Detection method: \code{"correlation"} (frontal channel
#'   correlation), \code{"kurtosis"} (excess kurtosis), \code{"spatial"}
#'   (spatial weight pattern), or \code{"iclabel"} (seven-class classifier).
...
eegICAdetect <- function(x, method = c("correlation", "kurtosis", "spatial",
  ...
  if (method == "iclabel") {
    lab <- eegICLabel(x, ica_assay = ica_assay)
    return(data.frame(
      component = lab$component,
      type = ifelse(lab$label == "brain", "neural", "artifact"),
      method = "iclabel",
      score = 1 - lab$brain,
