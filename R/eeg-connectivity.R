...
eegCoherence <- function(x, method = c("coherence", "imaginary"),
  ...
  .recordProv(x, input_assay = assay_name, output_assay = NA_character_,
              .package = "PhysioEEG")
}
...
#' @param assay_name Name of the input assay. If \code{NULL}, the default
#'   assay is used.
#' @param debiased Logical; also compute the unbiased debiased wPLI
#'   (default TRUE).
#' @return A data.frame with columns:
#'   \describe{
#'     \item{channel1}{Character or integer identifier of the first channel.}
...
#'     \item{wpli_debiased}{Numeric debiased wPLI (Vinck et al., 2011, Eq. 6),
#'       computed by the shared \code{\link[PhysioCore]{wpliEstimate}} estimator
#'       so it matches \code{PhysioCrossModal::weightedPLI}. It is unbiased and
#'       therefore distributes around 0 for independent signals (it is
#'       \emph{not} clamped to be non-negative).}
#'   }
#' @references
#' Lachaux, J. P., et al. (1999). Measuring phase synchrony in brain signals.
#' Human Brain Mapping, 8(4), 194-208.
...
#'   [eegPlotConnectivity()], \code{\link[PhysioCore]{wpliEstimate}}
#' @export
#' @examples
#' \dontrun{
#' pe <- make_eeg(n_time = 5000, n_channels = 4, sr = 500)
...
eegWPLI <- function(x, band = c(8, 13), window_sec = 2, overlap = 0.5,
  ...
  for (i in seq_len(n_channels - 1)) {
    for (j in (i + 1):n_channels) {
      ...
      # Raw wPLI and unbiased debiased wPLI (Vinck et al., 2011, Eq. 6) via the
      # shared PhysioCore estimator, so this matches PhysioCrossModal::weightedPLI.
      est <- PhysioCore::wpliEstimate(im_per_window, debiased = debiased)
      wpli_val <- est$wpli
      wpli_debiased <- est$wpli_debiased
...
# Channel labels (falls back to Ch1..Chn).
.conn_labels <- function(x, n) {
  cd <- SummarizedExperiment::colData(x)
  if ("label" %in% names(cd)) as.character(cd$label) else paste0("Ch", seq_len(n))
}

# Resolve a channel given as a label or an integer index.
.resolve_ch <- function(ch, labels) {
  if (is.numeric(ch)) return(as.integer(ch))
  idx <- which(labels == ch)
  if (!length(idx)) stop("channel not found: ", ch, call. = FALSE)
  idx[1]
}

# Fit an MVAR model (demeaned) and evaluate its transfer function on freqs.
.eeg_fit_mvar_freqs <- function(data, sr, order, method, freqs) {
  X <- sweep(data, 2, colMeans(data))
  fit <- PhysioCore::mvarFit(X, order = order, method = method)
  if (is.null(freqs)) freqs <- seq(0, sr / 2, length.out = 128L)
  list(fit = fit, sp = PhysioCore::mvarTransfer(fit, freqs, sr), freqs = freqs)
}

# DTF from the transfer function H. DTF[i,j,f] = |H_ij| / sqrt(sum_m |H_im|^2)
# (row / inflow normalized so sum_j DTF[i,j]^2 = 1). ffDTF normalizes across all
# frequencies instead. normalized = FALSE returns raw |H|.
.compute_dtf <- function(H, normalized, ffDTF) {
  n <- dim(H)[1]; nf <- dim(H)[3]
  mH <- Mod(H); dtf <- array(0, dim(H))
  if (ffDTF) {
    for (i in seq_len(n)) {
...
# PDC from the frequency-domain coefficient matrix Abar. PDC[i,j,f] =
# |Abar_ij| / sqrt(sum_m |Abar_mj|^2) (column / outflow normalized so
# sum_i PDC[i,j]^2 = 1). Generalized PDC weights each row by 1/sigma_i.
.compute_pdc <- function(Ab, sigma, generalized) {
  n <- dim(Ab)[1]; nf <- dim(Ab)[3]
  mA <- Mod(Ab); w <- if (generalized) 1 / sigma else rep(1, n)
  pdc <- array(0, dim(Ab))
  for (f in seq_len(nf)) for (j in seq_len(n)) {
    num <- mA[, j, f] * w
    denom <- sqrt(sum(num^2))
    pdc[, j, f] <- if (denom > 0) num / denom else 0
  }
  pdc
}

# Average an n x n x nfreq array over a frequency band (all freqs if band NULL).
.band_average <- function(arr, freqs, band) {
...
.store_connectivity <- function(x, matrix, array, freqs, method, band, order,
                                ch_labels) {
  md <- S4Vectors::metadata(x)
  md$connectivity <- list(matrix = matrix, array = array, frequencies = freqs,
                          method = method, band = band, directed = TRUE,
                          order = order, channels = ch_labels)
  S4Vectors::metadata(x) <- md
  x
}
...
#' transfer function \eqn{H(f)} of a multivariate autoregressive (MVAR) model
#' fitted with [PhysioCore::mvarFit()]. Because \eqn{H(f)} inverts the whole VAR
#' system, DTF reflects both direct and indirect pathways. The (default)
#' normalized DTF satisfies \eqn{\sum_j \mathrm{DTF}_{ij}(f)^2 = 1} for each
#' target \eqn{i} (inflow normalization); \code{ffDTF} normalizes across all
#' frequencies instead (Korzeniewska 2003).
#'
#' @param x A PhysioExperiment object with 2D EEG data (time x channels).
#' @param order MVAR model order, or \code{NULL} to select automatically.
#' @param freqs Numeric vector of frequencies in Hz (default: 128 points from 0
...
#' @param band Optional numeric length-2 band in Hz over which to average the
#'   stored connectivity matrix (default: all frequencies).
#' @param method MVAR estimator passed to [PhysioCore::mvarFit()]
#'   (default: \code{"ols"}).
#' @param assay_name Input assay name (default: the default assay).
#' @return The PhysioExperiment with \code{metadata(x)$connectivity} set to a
#'   list containing the band-averaged directed \code{matrix} (rows = targets,
#'   columns = sources), the frequency-resolved \code{array}
#'   (n_channels x n_channels x n_freqs), the \code{frequencies}, and the
#'   settings used.
#' @references
...
#' 65(3), 203-210.
#' @seealso [eegPDC()], [eegConditionalGC()], [eegConnectivityMatrix()],
#'   [PhysioCore::mvarFit()]
#' @export
#' @examples
#' \dontrun{
...
#' }
eegDTF <- function(x, order = NULL, freqs = NULL, normalized = TRUE,
                   ffDTF = FALSE, band = NULL, method = "ols",
                   assay_name = NULL) {
  stopifnot(inherits(x, "PhysioExperiment"))
  if (is.null(assay_name)) assay_name <- defaultAssay(x)
  data <- SummarizedExperiment::assay(x, assay_name)
  if (length(dim(data)) != 2) {
    stop("eegDTF requires 2D data (time x channels).", call. = FALSE)
  }
  sr <- samplingRate(x)
  ch_labels <- .conn_labels(x, ncol(data))
  mf <- .eeg_fit_mvar_freqs(data, sr, order, method, freqs)
  dtf <- .compute_dtf(mf$sp$H, normalized, ffDTF)
  dimnames(dtf) <- list(ch_labels, ch_labels, NULL)
  bm <- .band_average(dtf, mf$freqs, band)
  dimnames(bm) <- list(ch_labels, ch_labels)
  .store_connectivity(x, bm, dtf, mf$freqs, if (ffDTF) "ffDTF" else "dtf",
                      band, mf$fit$order, ch_labels)
}
...
#' coefficient matrix \eqn{\bar{A}(f)} of an MVAR model
#' ([PhysioCore::mvarFit()]). Unlike DTF, PDC reflects only \emph{direct}
...
#' satisfies \eqn{\sum_i \mathrm{PDC}_{ij}(f)^2 = 1} for each source \eqn{j}
...
#' @param freqs Numeric vector of frequencies in Hz (default: 128 points from 0
#'   to the Nyquist frequency).
#' @param generalized Use generalized PDC (default: \code{TRUE}).
#' @param band Optional numeric length-2 band in Hz over which to average the
#'   stored connectivity matrix (default: all frequencies).
#' @param method MVAR estimator passed to [PhysioCore::mvarFit()]
#'   (default: \code{"ols"}).
#' @param assay_name Input assay name (default: the default assay).
#' @return The PhysioExperiment with \code{metadata(x)$connectivity} set as in
#'   [eegDTF()] (a band-averaged directed \code{matrix}, the frequency-resolved
#'   \code{array}, \code{frequencies}, and settings).
#' @references
...
#' 463-474.
#' @seealso [eegDTF()], [eegConditionalGC()], [eegConnectivityMatrix()]
#' @export
#' @examples
#' \dontrun{
#' pe <- make_eeg(n_time = 4000, n_channels = 5, sr = 250)
...
#' }
eegPDC <- function(x, order = NULL, freqs = NULL, generalized = TRUE,
                   band = NULL, method = "ols", assay_name = NULL) {
  stopifnot(inherits(x, "PhysioExperiment"))
  if (is.null(assay_name)) assay_name <- defaultAssay(x)
  data <- SummarizedExperiment::assay(x, assay_name)
  if (length(dim(data)) != 2) {
    stop("eegPDC requires 2D data (time x channels).", call. = FALSE)
  }
  sr <- samplingRate(x)
  ch_labels <- .conn_labels(x, ncol(data))
  mf <- .eeg_fit_mvar_freqs(data, sr, order, method, freqs)
  sigma <- sqrt(diag(mf$fit$Sigma))
  pdc <- .compute_pdc(mf$sp$A, sigma, generalized)
  dimnames(pdc) <- list(ch_labels, ch_labels, NULL)
  bm <- .band_average(pdc, mf$freqs, band)
  dimnames(bm) <- list(ch_labels, ch_labels)
  .store_connectivity(x, bm, pdc, mf$freqs, if (generalized) "gpdc" else "pdc",
                      band, mf$fit$order, ch_labels)
}

#' Conditional (multivariate) Granger causality
#'
#' Computes conditional Granger causality from \code{source} to \code{target}
#' given a set of \code{conditioning} channels, using the log ratio of the
#' target's residual variance in a VAR fitted without the source
#' (\code{target + conditioning}) versus with it
#' (\code{target + source + conditioning}); both VARs share the same order
...
#'
#' @param x A PhysioExperiment object with 2D EEG data (time x channels).
#' @param target Target channel (label or index) - the effect.
#' @param source Source channel (label or index) - the putative cause.
#' @param conditioning Channels to condition on (labels or indices). Defaults to
#'   all remaining channels.
#' @param order MVAR model order, or \code{NULL} to select automatically on the
#'   full channel set.
#' @param method MVAR estimator passed to [PhysioCore::mvarFit()]
#'   (default: \code{"ols"}).
#' @param assay_name Input assay name (default: the default assay).
#' @return A list with the conditional GC \code{value} (in nats, non-negative in
#'   theory), the \code{target}, \code{source}, and \code{conditioning} labels,
#'   and the VAR \code{order} used.
#' @references
#' Barnett, L., & Seth, A. K. (2014). The MVGC multivariate Granger causality
#' toolbox. Journal of Neuroscience Methods, 223, 50-68.
#' @seealso [eegGrangerCausality()], [eegDTF()], [eegPDC()]
#' @export
#' @examples
#' \dontrun{
...
#' }
eegConditionalGC <- function(x, target, source, conditioning = NULL,
                             order = NULL, method = "ols", assay_name = NULL) {
  stopifnot(inherits(x, "PhysioExperiment"))
  if (is.null(assay_name)) assay_name <- defaultAssay(x)
  data <- SummarizedExperiment::assay(x, assay_name)
  if (length(dim(data)) != 2) {
    stop("eegConditionalGC requires 2D data (time x channels).", call. = FALSE)
  }
  ch_labels <- .conn_labels(x, ncol(data))
  ti <- .resolve_ch(target, ch_labels)
  si <- .resolve_ch(source, ch_labels)
  cond <- if (is.null(conditioning)) {
    setdiff(seq_along(ch_labels), c(ti, si))
  } else {
    vapply(conditioning, .resolve_ch, integer(1), ch_labels)
  }

  X <- sweep(data, 2, colMeans(data))
  full <- PhysioCore::mvarFit(X[, c(ti, si, cond), drop = FALSE],
                              order = order, method = method)
  reduced <- PhysioCore::mvarFit(X[, c(ti, cond), drop = FALSE],
                                 order = full$order, method = method)
  gc <- log(reduced$Sigma[1, 1] / full$Sigma[1, 1])

  list(value = gc, target = ch_labels[ti], source = ch_labels[si],
       conditioning = ch_labels[cond], order = full$order)
}
...
#'   coherence, \code{"plv"} for Phase Locking Value, \code{"wpli"} for weighted
#'   Phase Lag Index, or the directed measures \code{"dtf"} ([eegDTF()]) and
#'   \code{"pdc"} ([eegPDC()]) (default: \code{"coherence"}). For \code{"dtf"}
#'   and \code{"pdc"} the returned matrix is directed (rows = targets, columns =
#'   sources).
#' @param band Numeric vector of length 2 specifying the frequency band in Hz
...
eegConnectivityMatrix <- function(x, method = c("coherence", "plv", "wpli",
  ...
  } else if (method %in% c("dtf", "pdc")) {
    # Directed, frequency-resolved measures: return the band-averaged directed
    # matrix (rows = targets, columns = sources) from the MVAR fit.
    res <- if (method == "dtf") {
      eegDTF(x, order = dots$order, band = band, assay_name = dots$assay_name)
    } else {
      eegPDC(x, order = dots$order, band = band, assay_name = dots$assay_name)
    }
    conn_matrix <- S4Vectors::metadata(res)$connectivity$matrix
  }
