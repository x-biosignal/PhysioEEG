...
# Katz fractal dimension (Katz 1988) -- amplitude-only convention, matching the
# de-facto references (antropy / NeuroKit2 / pyeeg / mne-features). Delegates to
# the exported katzFD() so eegComplexity(measure = "katz_fd") is bit-for-bit
# comparable with those tools. (The earlier Euclidean-plane variant mixed the
# sample-index and amplitude axes' units and was not cross-tool comparable.)
.eeg_katz <- function(x) {
...
#' (Katz, 1988) -- a waveform-complexity measure derived from the total length of
#' the signal curve relative to its extent. Let \eqn{L} be the sum of the
#' absolute successive differences (the curve length), \eqn{a} their mean, and
#' \eqn{d} the maximum absolute distance from the first sample; then
#' \eqn{\mathrm{KFD} = \log_{10}(L/a) / \log_{10}(d/a)}. A smoother, more regular
#' signal has a lower KFD; a more convoluted (noise-like) waveform a higher one.
#'
#' This uses the amplitude-only convention adopted by the de-facto reference
#' implementations (Python \code{antropy}, \code{NeuroKit2}, \code{pyeeg},
#' \code{mne-features}), in which both the successive distances and the extent are
#' measured in the signal's amplitude units alone; it reproduces
#' \code{antropy.katz_fd} and \code{NeuroKit2}'s \code{fractal_katz} bit-for-bit.
#' (Katz's original definition uses the Euclidean distance in the
#' sample-index/amplitude plane, which mixes the two axes' units, is
#' scale-dependent, and is not comparable across tools; the amplitude-only form
#' is what the field uses and what makes KFD reproducible.)
#'
#' @param x A numeric vector (the time series).
#' @return A single numeric value, Katz's fractal dimension.
#'
#' @references
#' Katz, M. J. (1988). "Fractals and the analysis of waveforms."
#'   \emph{Computers in Biology and Medicine}, 18(3), 145--156.
#'   \doi{10.1016/0010-4825(88)90041-8}
#'
#' @seealso \code{\link{petrosianFD}} for the derivative-sign-change fractal view,
#'   \code{\link{svdEntropy}}, \code{\link{eegComplexity}} for the multi-measure
#'   wrapper.
#'
#' @export
#' @examples
...
katzFD <- function(x) {
  ...
  if (n < 3L) stop("`x` must have length >= 3.", call. = FALSE)
  dists <- abs(diff(x))
  L <- sum(dists)                          # total curve length (amplitude units)
  a <- mean(dists)                         # average successive distance
  d <- max(abs(x - x[1L]))                 # farthest distance from the first point
  if (a <= 0 || d <= 0) return(NA_real_)   # constant signal: KFD undefined
  log10(L / a) / log10(d / a)
}

#' Multiscale Entropy
#'
#' Computes the multiscale entropy (MSE; Costa, Goldberger & Peng, 2002) of a
#' one-dimensional signal: the sample entropy of the signal after coarse-graining
#' it at a series of time scales. At scale \eqn{s} the signal is partitioned into
#' non-overlapping windows of length \eqn{s}, each window is replaced by its mean,
#' and the sample entropy (embedding \code{m}, tolerance \code{r} times the SD of
#' the ORIGINAL signal, held fixed across scales) of that coarse-grained series is
#' the MSE at scale \eqn{s}. A signal whose MSE stays high or rises at coarse
#' scales carries structure across time scales; white noise, by contrast, has an
#' MSE that falls monotonically from scale 1.
#'
#' @param x A numeric vector (the time series).
#' @param m Embedding dimension for the sample entropy (default 2).
#' @param r Tolerance as a fraction of the ORIGINAL signal's SD (default 0.2),
#'   held fixed across all scales (Costa's convention).
#' @param scales Integer vector of time scales (default \code{1:8}).
#' @return A named numeric vector of sample entropies, one per scale
#'   (\code{scale1}, \code{scale2}, ...). \code{NA} for a scale whose
#'   coarse-grained series is too short or yields no template matches.
#'
#' @references
#' Costa, M., Goldberger, A. L., & Peng, C.-K. (2002). "Multiscale entropy
#'   analysis of complex physiologic time series." \emph{Physical Review Letters},
#'   89(6), 068102. \doi{10.1103/PhysRevLett.89.068102}
#'
#' @seealso \code{\link{eegComplexity}} for the multi-measure wrapper;
#'   PhysioMoCap's \code{sampleEntropy} for the single-scale measure.
#'
#' @export
#' @examples
#' set.seed(1); multiscaleEntropy(rnorm(1000), scales = 1:5)  # noise: falls with scale
multiscaleEntropy <- function(x, m = 2L, r = 0.2, scales = 1:8) {
  x <- as.numeric(x)
  stopifnot(is.numeric(scales), length(scales) >= 1L, all(scales >= 1))
  rr <- r * stats::sd(x)
  if (!is.finite(rr) || rr <= 0) stop("`x` has (near) zero variance; MSE is undefined.", call. = FALSE)
  out <- vapply(as.integer(scales), function(s) {
    cg <- if (s == 1L) x else {
      nb <- length(x) %/% s
      if (nb < 1L) return(NA_real_)
      colMeans(matrix(x[seq_len(nb * s)], nrow = s))   # non-overlapping-window means
    }
    .eeg_sampen(cg, m, r_abs = rr)                      # tolerance fixed on the original SD
  }, numeric(1))
  names(out) <- paste0("scale", as.integer(scales))
  out
}
