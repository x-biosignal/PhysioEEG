...
#' \eqn{LI = (R - L) / (|R| + |L|)} is computed from a right- and a
#' left-hemisphere channel (or ROI), bounded from -1 to 1. With
#' \code{method = "erd"} the activation is the event-related desynchronization
#' magnitude from [eegMotorImagery()] (a positive LI means the right hemisphere
#' desynchronizes more, i.e. is more active); with \code{method = "power"} it is
#' the raw band power (a positive LI means the right hemisphere has more power).
#' A symmetric input gives \eqn{LI \approx 0}.
#'
#' @param x A PhysioExperiment object with epoched (3D) EEG data
#'   (time x channels x trials).
#' @param left_ch Character label(s) of the left-hemisphere channel or ROI
#'   (default: \code{"C3"}).
#' @param right_ch Character label(s) of the right-hemisphere channel or ROI
#'   (default: \code{"C4"}).
#' @param band Numeric length-2 frequency band in Hz (default: \code{c(8, 13)},
#'   the mu rhythm).
#' @param method Activation measure: \code{"erd"} (desynchronization magnitude,
#'   via [eegMotorImagery()]) or \code{"power"} (raw band power).
#' @param baseline_fraction Fraction of each trial used as the ERD baseline
#'   (default: 0.25); only used by \code{method = "erd"}.
#' @param conf_level Confidence level for the summary interval (default: 0.95).
#' @param reliability Named list of reliability indices attached to the summary
#'   biomarker (default: \code{icc} and \code{sem} placeholders).
#' @param assay_name Input assay name (default: the default assay).
#' @return A list with \code{per_trial} (an \code{n_trials} x 1 matrix of
#'   laterality indices) and \code{summary} (a
#'   \code{\link[PhysioCore]{PhysioBiomarker}} holding the mean LI with its
...
#'
#' Bai, O., et al. (2005). Asymmetric spatiotemporal patterns of event-related
#' desynchronization preceding voluntary sequential finger movements. Clinical
#' Neurophysiology, 116(5), 1213-1221.
#' @seealso [eegMotorImagery()], [eegCSP()], [PhysioCore::physioBiomarker()]
#' @export
#' @examples
#' \dontrun{
#' pe <- make_eeg_bci(n_trials = 20, n_channels = 8, sr = 256)
...
#' li$summary
#' }
eegLateralization <- function(x, left_ch = "C3", right_ch = "C4",
                              band = c(8, 13), method = c("erd", "power"),
                              baseline_fraction = 0.25, conf_level = 0.95,
  ...
  data <- SummarizedExperiment::assay(x, assay_name)
  if (length(dim(data)) != 3) {
    stop("eegLateralization requires epoched (3D) data (time x channels x trials).",
         call. = FALSE)
  }
  n_time <- dim(data)[1]; n_channels <- dim(data)[2]; n_trials <- dim(data)[3]
  cd <- SummarizedExperiment::colData(x)
  ch_labels <- if ("label" %in% colnames(cd)) as.character(cd$label) else
    paste0("Ch", seq_len(n_channels))

  left_idx <- which(ch_labels %in% left_ch)
  right_idx <- which(ch_labels %in% right_ch)
  if (!length(left_idx)) {
    stop("left_ch not found in channel labels: ", paste(left_ch, collapse = ", "),
         call. = FALSE)
  }
  if (!length(right_idx)) {
    stop("right_ch not found in channel labels: ", paste(right_ch, collapse = ", "),
         call. = FALSE)
  }

  # Per-trial hemispheric activation, averaged across ROI channels.
  if (method == "erd") {
    mi <- eegMotorImagery(x, bands = list(li = band),
                          baseline_fraction = baseline_fraction,
                          assay_name = assay_name)
    erd <- S4Vectors::metadata(mi)$erd_ers            # n_trials x (n_ch), "<ch>_li"
    # desynchronization magnitude: ERD is a negative percentage, so negate.
    a_left <- -rowMeans(erd[, paste0(ch_labels[left_idx], "_li"), drop = FALSE])
    a_right <- -rowMeans(erd[, paste0(ch_labels[right_idx], "_li"), drop = FALSE])
  } else {
    band_pow <- function(idx) {
      vals <- vapply(idx, function(ch) vapply(seq_len(n_trials), function(tr) {
        mean(.fir_bandpass(data[, ch, tr], samplingRate(x), band[1], band[2])^2)
      }, numeric(1)), numeric(n_trials))
      if (is.matrix(vals)) rowMeans(vals) else vals
    }
    a_left <- band_pow(left_idx); a_right <- band_pow(right_idx)
  }

  li <- (a_right - a_left) / (abs(a_right) + abs(a_left) + 1e-20)
  per_trial <- matrix(li, ncol = 1, dimnames = list(NULL, "LI"))

  m <- mean(li)
  s <- stats::sd(li)
  ci <- if (n_trials > 1L && is.finite(s) && s > 0) {
    m + c(-1, 1) * stats::qt((1 + conf_level) / 2, n_trials - 1L) * s / sqrt(n_trials)
  } else numeric(0)

  ver <- tryCatch(as.character(utils::packageVersion("PhysioEEG")),
                  error = function(e) NA_character_)
  summary_bm <- PhysioCore::physioBiomarker(
    value = m, name = "LI", unit = "index", ci = ci, reliability = reliability,
    provenance = list(assay = assay_name,
                      band = sprintf("%g-%gHz", band[1], band[2]),
                      method = method, software_version = ver),
    interpretation = sprintf("right(%s) vs left(%s); positive = right-dominant",
                             paste(right_ch, collapse = "/"),
                             paste(left_ch, collapse = "/")))

  list(per_trial = per_trial, summary = summary_bm)
}
...
eegBCIclassify <- function(x, features = NULL, labels,
  ...
  if (!is.null(cv_folds)) {
    ...
    warning("eegBCIclassify() cross-validation extracts features on the full ",
            "dataset before splitting into folds, which leaks label/data ",
            "information into the test folds and inflates accuracy (Varoquaux ",
            "et al., 2017). This path is deprecated; use eegDecode() for a ",
            "leakage-free per-fold pipeline.", call. = FALSE)
