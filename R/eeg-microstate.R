...
#'
#' Computes the spatial dispersion across channels at each time point. The
#' population definition uses denominator C, matching the established
#' microstate implementation; the sample definition uses C - 1.
#'
#' @param data Finite numeric matrix with time points in rows and channels in
#'   columns.
#' @param definition Either \code{"population"} or \code{"sample"}.
#' @return Numeric vector with one GFP value per row.
#' @keywords internal
#' @noRd
.eeg_global_field_power <- function(
    data,
    definition = c("population", "sample")) {
  definition <- match.arg(definition)

  if (!is.matrix(data) || !is.numeric(data) ||
      nrow(data) < 1L || ncol(data) < 1L) {
    stop("data must be a non-empty numeric time-by-channel matrix.",
         call. = FALSE)
  }
  if (any(!is.finite(data))) {
    stop("data must contain only finite values.", call. = FALSE)
  }
  if (definition == "sample" && ncol(data) < 2L) {
    stop("Sample GFP requires at least two channels.", call. = FALSE)
  }

  row_means <- rowMeans(data)
  centered <- data - row_means
  denominator <- if (definition == "population") ncol(data) else ncol(data) - 1L
  sqrt(rowSums(centered * centered) / denominator)
}
...
eegMicrostates <- function(x, n_states = 4,
  ...
  gfp <- .eeg_global_field_power(data, definition = "population")
