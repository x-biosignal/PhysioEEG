...
#'
#' Computes the delta-to-alpha band-power ratio per channel (or per region), a
#' quantitative-EEG stroke marker whose elevation tracks ischaemic slowing
#' (Finnigan et al. 2016). Band powers use a Welch estimator for a stable
#' spectrum. Each ratio is returned as a reliability-characterised
#' \code{\link[PhysioCore]{physioBiomarker}} carrying provenance (band, method,
#' software version), reliability placeholders, and - when age (and montage) are
#' supplied - a published reference range from
#' \code{\link[PhysioCore]{normativeLookup}} so the value prints its normative
#' percentile.
#'
#' @param x A PhysioExperiment object with 2D EEG data.
#' @param delta Numeric length-2 delta band in Hz (default: \code{c(1, 4)}).
#' @param alpha Numeric length-2 alpha band in Hz (default: \code{c(8, 13)}).
#' @param regions Optional named list mapping a region name to a character
#'   vector of channel labels; band powers are averaged within each region. When
#'   \code{NULL} (default) one biomarker is returned per channel.
#' @param method Spectral estimator passed to the band-power routine
#'   (default: \code{"welch"}).
#' @param age Optional numeric age used to select the normative reference row.
#' @param montage Optional character montage for the normative lookup.
...
#' @return A named list of \code{\link[PhysioCore]{PhysioBiomarker}} objects, one
#'   per channel or region.
#' @references
...
#' @seealso [eegBSI()], [eegSlowing()], [eegQEEG()],
#'   [PhysioCore::physioBiomarker()], [PhysioCore::normativeLookup()]
#' @export
#' @examples
#' \dontrun{
...
#' dar[["C3"]]
#' }
eegDAR <- function(x, delta = c(1, 4), alpha = c(8, 13), regions = NULL,
                   method = "welch", age = NULL, montage = NULL,
                   reliability = list(icc = NA_real_, sem = NA_real_),
                   assay_name = NULL) {
  stopifnot(inherits(x, "PhysioExperiment"))
  stopifnot(is.numeric(delta), length(delta) == 2, delta[1] < delta[2])
  stopifnot(is.numeric(alpha), length(alpha) == 2, alpha[1] < alpha[2])

  if (is.null(assay_name)) assay_name <- defaultAssay(x)
  data <- SummarizedExperiment::assay(x, assay_name)
  if (!is.matrix(data)) {
    stop("eegDAR requires a 2D (time x channels) assay.", call. = FALSE)
  }
  sr <- samplingRate(x)
  ...
  ver <- tryCatch(as.character(utils::packageVersion("PhysioEEG")),
                  error = function(e) NA_character_)
  band_txt <- sprintf("delta[%g-%g]/alpha[%g-%g]Hz",
                      delta[1], delta[2], alpha[1], alpha[2])
  nl <- PhysioCore::normativeLookup("DAR", age = age, montage = montage)
  refrange <- if (isTRUE(nl$matched) && is.finite(nl$mean) && is.finite(nl$sd))
    c(nl$mean - 1.959964 * nl$sd, nl$mean + 1.959964 * nl$sd) else numeric(0)

  units <- if (is.null(regions)) {
    stats::setNames(as.list(seq_len(ncol(data))), ch_labels)
  } else {
    stopifnot(is.list(regions))
    lapply(regions, function(chs) which(ch_labels %in% chs))
  }

  make_one <- function(idx, label) {
    if (!length(idx)) return(NULL)
    dp <- mean(vapply(idx, function(i)
      .compute_band_power(data[, i], sr, delta[1], delta[2], method), numeric(1)))
    ap <- mean(vapply(idx, function(i)
      .compute_band_power(data[, i], sr, alpha[1], alpha[2], method), numeric(1)))
    dar <- if (ap > .Machine$double.eps) dp / ap else Inf
    PhysioCore::physioBiomarker(
      value = dar, name = "DAR", unit = "ratio",
      reference_range = refrange, reliability = reliability,
      provenance = list(assay = assay_name, band = band_txt,
                        method = method, software_version = ver),
      interpretation = paste0("channel/region: ", label))
  }
  out <- Map(make_one, units, names(units))
...
#'
#' Quantifies left/right spectral asymmetry across hemispheric electrode pairs, a
#' quantitative-EEG stroke marker (van Putten 2007; Sheorajpanday et al. 2011).
#' The global \code{bsi} is the spectrum-averaged absolute relative difference
#' between summed right- and left-hemisphere power, bounded from 0 (symmetric) to
#' 1 (one hemisphere silent). The pairwise-derived \code{pdbsi} averages the same
#' relative difference per hemispheric pair; with \code{directed = TRUE} it keeps
#' the sign (positive = right-dominant). Both are returned as
#' reliability-characterised \code{\link[PhysioCore]{physioBiomarker}} objects
#' with provenance and, for the global BSI, a normative reference range.
#'
#' @param x A PhysioExperiment object with 2D EEG data.
#' @param pairs List of length-2 character vectors, each a \code{c(right, left)}
#'   hemispheric electrode pair. Defaults to the standard 10-20 hemispheric
#'   pairs.
#' @param band Numeric length-2 frequency band in Hz (default: \code{c(1, 25)}).
#' @param directed Logical; when \code{TRUE} the pairwise index keeps its sign
#'   (default: \code{FALSE}).
#' @param method Spectral estimator (default: \code{"welch"}).
#' @param age Optional numeric age for the normative lookup.
#' @param montage Optional character montage for the normative lookup.
#' @param reliability Named list of reliability indices to attach (default:
#'   \code{icc} and \code{sem} placeholders).
#' @param assay_name Input assay name (default: the default assay).
#' @return A named list with two \code{\link[PhysioCore]{PhysioBiomarker}}
#'   objects: \code{bsi} (global, from 0 to 1) and \code{pdbsi}
#'   (pairwise-derived, signed when \code{directed = TRUE}).
#' @references
#' van Putten, M. J. A. M. (2007). The revised brain symmetry index. Clinical
#' Neurophysiology, 118(11), 2362-2367.
#'
#' Sheorajpanday, R. V. A., et al. (2011). Quantitative EEG in ischemic stroke:
#' correlation with functional status after 6 months. Clinical Neurophysiology,
#' 122(5), 874-883.
#' @seealso [eegDAR()], [eegAsymmetry()], [PhysioCore::physioBiomarker()]
#' @export
#' @examples
#' \dontrun{
#' pe <- make_eeg(n_time = 5000, n_channels = 19, sr = 250)
...
#' res$bsi
#' }
eegBSI <- function(x, pairs = NULL, band = c(1, 25), directed = FALSE,
                   method = "welch", age = NULL, montage = NULL,
                   reliability = list(icc = NA_real_, sem = NA_real_),
                   assay_name = NULL) {
  stopifnot(inherits(x, "PhysioExperiment"))
  ...
  data <- SummarizedExperiment::assay(x, assay_name)
  if (!is.matrix(data)) {
    stop("eegBSI requires a 2D (time x channels) assay.", call. = FALSE)
  }
  sr <- samplingRate(x)
  cd <- SummarizedExperiment::colData(x)
  ch_labels <- if ("label" %in% colnames(cd)) as.character(cd$label) else
  ...
  r_psd <- list(); l_psd <- list(); freq <- NULL
  pair_vals <- numeric(0); used_pairs <- character(0)
  for (p in pairs) {
    ri <- which(ch_labels == p[1]); li <- which(ch_labels == p[2])
    if (!length(ri) || !length(li)) next
    pr <- .welch_psd(data[, ri[1]], sr); pl <- .welch_psd(data[, li[1]], sr)
    if (is.null(freq)) freq <- pr$freq
    r_psd[[length(r_psd) + 1L]] <- pr$psd
    l_psd[[length(l_psd) + 1L]] <- pl$psd
    m <- pr$freq >= band[1] & pr$freq <= band[2]
    d <- (pr$psd[m] - pl$psd[m]) / (pr$psd[m] + pl$psd[m] + 1e-20)
    pair_vals <- c(pair_vals, mean(if (directed) d else abs(d)))
    used_pairs <- c(used_pairs, paste0(p[1], "-", p[2]))
  }
  if (!length(r_psd)) {
    stop("No hemispheric pairs matched the channel labels.", call. = FALSE)
  }

  m <- freq >= band[1] & freq <= band[2]
  r_tot <- Reduce(`+`, r_psd); l_tot <- Reduce(`+`, l_psd)
  bsi_val <- mean(abs((r_tot[m] - l_tot[m]) / (r_tot[m] + l_tot[m] + 1e-20)))
  pdbsi_val <- mean(pair_vals)

  ver <- tryCatch(as.character(utils::packageVersion("PhysioEEG")),
                  error = function(e) NA_character_)
  prov <- list(assay = assay_name, band = sprintf("%g-%gHz", band[1], band[2]),
               method = method, software_version = ver)
  nl <- PhysioCore::normativeLookup("BSI", age = age, montage = montage)
  refrange <- if (isTRUE(nl$matched) && is.finite(nl$mean) && is.finite(nl$sd))
    c(nl$mean - 1.959964 * nl$sd, nl$mean + 1.959964 * nl$sd) else numeric(0)

  list(
    bsi = PhysioCore::physioBiomarker(
      value = bsi_val, name = "BSI", unit = "index",
      reference_range = refrange, reliability = reliability, provenance = prov,
      interpretation = sprintf("%d hemispheric pairs", length(used_pairs))),
    pdbsi = PhysioCore::physioBiomarker(
      value = pdbsi_val, name = if (directed) "pdBSI_directed" else "pdBSI",
      unit = "index", reliability = reliability, provenance = prov,
      interpretation = paste(used_pairs, collapse = ",")))
}
...
#' @param nperseg Segment length in samples (default: about a one-second
#'   power-of-two segment, capped at the signal length).
#' @param noverlap Overlap in samples (default: half of \code{nperseg}).
#' @return A list with \code{freq} (Hz) and \code{psd} (one-sided).
#' @keywords internal
.welch_psd <- function(signal, sr, nperseg = NULL, noverlap = NULL) {
  n <- length(signal)
  # ~2-second segments give ~0.5 Hz resolution (adequate for delta/theta bands)
  # while still averaging several windows for a stable estimate.
  if (is.null(nperseg)) nperseg <- min(n, as.integer(round(2 * sr)))
  nperseg <- as.integer(min(nperseg, n))
  if (nperseg < 4L) nperseg <- as.integer(n)
  if (is.null(noverlap)) noverlap <- nperseg %/% 2L
  step <- max(1L, nperseg - as.integer(noverlap))
  w <- 0.5 - 0.5 * cos(2 * pi * (seq_len(nperseg) - 1) / (nperseg - 1))
  u <- sum(w^2)
  starts <- seq(1L, n - nperseg + 1L, by = step)
  if (!length(starts)) starts <- 1L
  acc <- 0
  for (a in starts) acc <- acc + Mod(stats::fft(signal[a:(a + nperseg - 1)] * w))^2
  acc <- acc / length(starts)
  half <- seq_len(nperseg %/% 2L + 1L)
  psd <- acc[half] / (sr * u)
  if (length(psd) > 2L) psd[2:(length(psd) - 1L)] <- psd[2:(length(psd) - 1L)] * 2
  list(freq = (half - 1L) * sr / nperseg, psd = psd)
}

#' Compute band power for a single signal (Welch PSD)
#'
#' Integrates a Welch (averaged-periodogram) power spectral density over the
#' band, replacing the earlier single-window FFT for a lower-variance estimate.
#'
#' @param signal Numeric vector of the input signal.
#' @param sr Sampling rate in Hz.
#' @param low Low frequency bound in Hz.
#' @param high High frequency bound in Hz.
#' @param method Spectral estimator; currently \code{"welch"}.
#' @param nperseg Optional Welch segment length in samples.
#' @return Numeric scalar of the integrated spectral power in the band.
#' @keywords internal
.compute_band_power <- function(signal, sr, low, high, method = "welch",
                                nperseg = NULL) {
  ps <- .welch_psd(signal, sr, nperseg = nperseg)
  m <- ps$freq >= low & ps$freq <= high
  if (!any(m)) return(0)
  df <- if (length(ps$freq) > 1L) ps$freq[2] - ps$freq[1] else sr / length(signal)
  sum(ps$psd[m]) * df
}
